...
**v6.1 更新重点**：整合 Gemini 架构审议意见，补充回滚策略、测试计划、风险评估。
...
### 2.5 额外风险项 (Gemini 审议补充)

1. **状态泄露 (State Leakage)**: RuntimeLoop 重置或 `BUDGET_EXCEEDED` 强制总结时，MMU Context Stack 必须正确压平或清理，防止下一个 Task/Session 误读残余信息。
2. **中断原子性**: RuntimeLoop 必须保证任意 step 之后接收中断信号时，MMU 状态可重现且一致（不能出现半写的 tool_use 没有配对 tool_result）。
3. **资源审计散落**: Token 消耗、LLM 调用成本、工具执行时间等审计逻辑散落在 OS 和 VCPU 中，长期应收敛到统一的 ResourceMonitor。
...
- **策略变更**: 压缩触发逻辑从 `_run_process` 剥离。初期由 RuntimeLoop 直接调用，后续如有 3+ 个 hook 消费者再抽象为插件机制。
...
### Phase 2: Runtime Loop 统一 (Critical — 深水区)
- **Step 2.1**: 创建 `core.process.factory.py`，统一 `spawn` 和 `chat` 的组件组装逻辑。
...
- **回滚策略**: 保留旧 `_run_process` 实现，通过 `config.use_unified_runtime` Feature Flag 切换。确认新循环在 harbor benchmark 和 web-ui 场景下稳定后再删除旧代码。

### Phase 3: AgentOS 瘦身
- 将 `AgentOS.run`, `chat`, `run_stream` 重写为调用 `RuntimeLoop`。
- 确认 Feature Flag 稳定后，删除旧的 `_run_process` 和 `run_stream` 实现。

## 6. 目录结构变更
```text
src/nimbus/
├── agentos.py            # Facade (<300行)
├── core/
│   ├── process/
...
### 7.1 一致性测试
- 针对同一 Goal，对比 `_run_process`（旧）和 `RuntimeLoop`（新）的最终 `ToolResult` 和中间 Trace
- 覆盖场景：单工具调用、多工具并发、子进程 spawn、chat 多轮对话

### 7.2 回归测试
- Harbor benchmark（evoeval@1.0, terminal-bench-sample@2.0）作为重构基准线
- Web-UI 端到端：Explorer/Implementer/Architect/Tester 各触发一次，验证 SSE 流和结果正确性

### 7.3 压力测试
- 3 并发 session（参考 v3-beta-2 压力测试记录）
- 高频中断：session 运行中连续发送 interrupt 信号
- IPC 注入：sub-agent 运行中向其 inbox 注入消息，验证不影响终止

### 7.4 风险监控
- 性能基准：重构前后对比单次 step 延迟（预期 <1ms 额外开销，子系统实例化成本可忽略）
- Context 一致性：重构后运行长对话（20+ 轮），检查 MMU 状态无泄露
