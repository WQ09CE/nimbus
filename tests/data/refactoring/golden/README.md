...
# Make API calls using new_api()
response = client.new_api("/users", method="GET")
print(response)

# Create a user
user = client.new_api("/users", method="POST", data={
    "username": "john",
...
- `new_api(endpoint, method="GET", data=None, headers=None)`: Make an API request
...
# Using new_api directly
response = client.new_api("/health")
