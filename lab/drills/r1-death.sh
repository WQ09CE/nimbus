...
lastev() { tail -1 "$SESS/$1.jsonl" | grep -oE '"type": "[^"]+"' | cut -d'"' -f4; }
watch() { local n=$1 T0=$(date +%s); for i in $(seq 5 5 "$n"); do sleep 5
  echo "   t+$(( $(date +%s) - T0 ))s pod-a=$(systemctl --user is-active lab-nimbus@a) vc=$(vc) steps: $(steps)| log-tail=$(lastev "$SID") | pod-b view: $(curl -s "127.0.0.1:8001/api/v1/sessions/$SID/status" | grep -oE '"running":[a-z]+,"status":"[a-z]+"') | ledger: $(ledger)"; done; }
...
seqdump() { python3 - "$SESS/$SID.jsonl" "$1" <<'PY'
