...
User request: {query}"""
        ...
        async for event in agent.run_stream(input_text):
            ...
            if event_type == "planning":
                yield sse_format(emit_event(task_id, "run_log", data=event.get("content", "Planning...")))
                ...
                tool_id = event.get("tool_use_id") or current_tool.get("id", f"call_{tool_call_counter}")
...
async def resume_agent_flow(
    ...
    """
    yield sse_format(emit_event(task_id, "run_log", data=f"Execution result received: {len(exec_result)} chars"))
    
    # Add exec result to context and continue
    # For MVP, just return success
    yield sse_format(emit_event(
        task_id,
        "agent_output", 
